...
pcl::PointCloud<myPointXYZRID> transform(pcl::PointCloud<myPointXYZRID> pc, Eigen::Matrix4f T)
{
	//Eigen::Affine3f transf = pcl::getTransformation(x, y, z, rot_x, rot_y, rot_z);
	pcl::PointCloud<myPointXYZRID> new_cloud;
	pcl::transformPointCloud(pc, new_cloud, T);
	return new_cloud;
}
